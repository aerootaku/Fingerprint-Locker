...
if(isset($_POST['delete'])){
    $delete = db_delete('tbl_client', $data = array("id"=>$_GET['id']));
        if(isset($delete)){
            ...
            $update = db_update('tbl_locker', $datas = array("status"=>"Active"), $where = array("locker_no"=>$_GET['locker_no']));
        }
...
if(isset($_POST['edit'])){
    $edit = db_update('tbl_client', $data = array("firstname"=>$_POST['firstname'], "lastname"=>$_POST['lastname'], "email"=>$_POST['email'], "gender"=>$_POST['gender'], "contact"=>$_POST['contact']), $data = array("id"=>$_GET['id']));
    if(isset($edit)){
        $_SESSION['toastr'] = array(
            'type'=>'info',
...
if(isset($_POST['reset'])){
    $reset = db_update('tbl_client', $data = array("pin_no"=>$_POST['pin']), array("id"=>$_GET['id']));
    if(isset($reset)){
...
<div id="app">
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="table table-bordered table-hover data-tables"
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $row['contact']; ?></td>
                                            <td>
                                                <?php if($row['status']=='Active') { $status = "text-success"; $desc = "Active";  } else {$status = "text-danger"; $desc = "Inactive"; }  ?>
                                                <span class="icon icon-circle s-12  mr-2 <?= $status; ?>"></span> <?= $desc; ?>
                                            </td>
                                            <td>
                ...
                <div class="modal-body">
                    <fieldset>
                        <div class="form-group">
                            <label>Pin</label>
                            <input type="text" class="form-control" name="pin" value="<?= $row['pin_no']; ?>" />
                        </div>
                        <div class="form-group">
    ...
    <div id="delete<?= $id; ?>" class="modal fade" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form action="?id=<?= $id; ?>&locker_no=<?= $row['locker_no']; ?>" method="POST">
                <div class="modal-content">
                <div class="modal-header">
    ...
    <div id="reset<?= $id; ?>" class="modal fade" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form action="?id=<?= $id; ?>" method="POST">
                <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Reset Pin</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <fieldset>
                        <div class="form-group">
                           <h4>Reseting the pin will change the pin to: <strong> <?php echo $rand = rand_string(8); ?></strong></h4>
                            <input type="hidden" name="pin" value="<?php echo $rand; ?>" />
                        </div>
                    </fieldset>
